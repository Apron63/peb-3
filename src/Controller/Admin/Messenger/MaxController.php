...
class MaxController extends MobileController
{
    public function __construct(
        private readonly UserSenderService $userSenderService,
    ) {}

    #[Route('/admin/user/max/resend/{id<\d+>}/', name: 'admin_user_max_resend')]
    public function maxResend(User $user): RedirectResponse
    {
        if (empty($user->getMobilePhone())) {
            $this->addFlash('error', 'Не задан номер телефона');
        } elseif (!$user->isMaxConfirmed()) {
            $this->addFlash('error', 'Не задано согласие на рассылку');
        } else {
            $result = $this->userSenderService->resendToMax($user);
